...
**Requisitos:**
- Crear un archivo de disco virtual de exactamente 10 MB en `$HOME/laboratorio/storage/disco_virtual.img` usando `dd`.
- Formatear el archivo con `mkfs.ext4`.
- Montar el disco en `$HOME/laboratorio/storage/montaje`.
...
- El archivo `$HOME/laboratorio/storage/disco_virtual.img` existe.
- El sistema de archivos es ext4.
- El punto de montaje `$HOME/laboratorio/storage/montaje` está activo.

**Pregunta de selección múltiple:**

> Después de ejecutar `dd if=/dev/zero of=$HOME/laboratorio/storage/disco_virtual.img bs=1M count=10`, ¿cuál es el tamaño exacto en bytes del archivo creado?
...
**Cálculo:** 10 MB × 1 048 576 bytes/MiB = 10 485 760 bytes. Este valor se obtiene ejecutando: `stat -c%s $HOME/laboratorio/storage/disco_virtual.img`.
...
**Requisitos:**
- Montar `$HOME/laboratorio/storage/disco_virtual.img` en `$HOME/laboratorio/storage/montaje`.
...
**Criterios de validación:**
- `$HOME/laboratorio/storage/montaje/archivo_prueba.txt` existe.
